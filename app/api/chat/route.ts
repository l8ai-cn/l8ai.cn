...
【工程引擎】也称 Do 引擎，支撑真实交付：
Doops（部署运维与远程执行）、DoAgent（Agent 编排与任务执行）、DoDNS（域名证书与入口路由治理）、DoSQL（数据查询、指标口径与分析取数）、DoSecurity（权限、安全、审计与风险响应）、DoProject（项目计划、任务协同、交付证据与复盘资产）。

L8AI 还提供企业 AI 生产化课程、实践案例和博客洞察。
...
const SITE_ANSWERS = [
  ...
  {
    patterns: [/do 引擎/i, /do引擎/i, /xdo/i, /doops/i, /doagent/i, /dodns/i, /dosql/i, /dosecurity/i, /doproject/i, /工具/, /engine/i, /sql/i],
    answer:
      "工程引擎也称 Do 引擎，是 L8AI 的工程工具系列，用来支撑真实交付：Doops 负责部署运维与远程执行，DoAgent 负责 Agent 编排与任务执行，DoDNS 负责域名证书与入口路由，DoSQL 负责数据查询、指标口径和分析取数，DoSecurity 负责权限、安全、审计和风险响应，DoProject 负责计划、任务、验收证据和复盘资产。",
  },
  {
...
function createConfigurationMessage(question: string) {
  return (
    ...
    "当前模型服务还没有配置 MINIMAX_API_KEY，我可以先回答 L8AI 官网基础问题，例如产品矩阵、八维方法、课程体系、工程引擎和预约咨询。更深入的开放问题请联系 contact@l8ai.cn。"
  )
}
